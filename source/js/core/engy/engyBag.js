        ...
        if (node in this.bag) {
            for (j in props) {
                if (typeof props[j] == 'function') {
                    props[j] = SB.util.delegate(props[j], node);
                }
                this.bag[node][j] = props[j];
            }
        } else {
            this.bag[node] = props;
        }
...
SB.engy.bag.set(n, {getParent : function () {return this.parentNode;}, prop2 : 'federico ghedina'});
SB.engy.bag.get(n, 'getParent')();
*/