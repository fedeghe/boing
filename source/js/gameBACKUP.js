~function () {
	...
	_gameProto = SB.Game.prototype;
	

	_gameProto._currentPowerup = false;
	
	_gameProto._icon = false;


/*
	DDpowerups/list.jsDD
*/	
	_gameProto.start = function () {
	 ...
	 */
	_gameProto.createPlayground = function () {
	 ...
	 * @return {[type]} [description]
	 */
	_gameProto.renderPanels = function () {
		var G = this,
			life = {
				tag : "i",
				attrs : {'class' : 'life'}
					...
					content : [{
						...
						cb : function () {
							var self = this,
								$elf = self.node,
								close = function () {
									SB.css.style(self.parent.parent.node, {display: 'none'});
									SB.Channel('boing').pub('closeHelp');
								}
							SB.events.onEsc(close);
							SB.events.on($elf, _bEvent, close);
							SB.Channel('boing').sub('openHelp', function () {
								SB.css.style(self.parent.parent.node, {display: 'block'});
							});
							this.done();
						}
					},{
						...
						content : [{
							tag : "li",
							html : "make the Staila bounce as long & high as possible"
						},{
							tag : "li",
